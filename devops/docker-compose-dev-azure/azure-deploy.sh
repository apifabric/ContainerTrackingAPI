...
resourcename="containertrackingapi"  # lower case, only
resourcegroup="containertrackingapi_rg"
...
# git clone https://github.com/apilogicserver/containertrackingapi.git
# cd containertrackingapi
...
az group create --name containertrackingapi_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group containertrackingapi_rg --sku S1 --is-linux
az container create --resource-group containertrackingapi_rg --name containertrackingapi --image apilogicserver/containertrackingapi:latest --dns-name-label containertrackingapi --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//containertrackingapi.westus.azurecontainer.io:5656'
